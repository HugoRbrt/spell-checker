...
bool possede_unique_fils(noeudPrefixe p){
  if(!possede_fils(p)){return false;}
  if(possede_frere(p->fils)){return false;}
  return true;
}
arbrePrefixe add_fils(arbrePrefixe source , char c){
  ...
  while(!possede_frere(p)){
    if(p->valeur == c){return p;}
    p=p->frere;
  }
  if(p->valeur!=c){
    if(possede_unique_fils(source)){source->valeur=strcat(source->valeur,c);} //dans le cas où le noeud source ne possède qu'un seul fils
    else{p->frere = creer_noeud(c);return p->frere;}
  }
...
bool est_present_arbrePrefixe(arbrePrefixe a, char* s){
  ...
  for(k=0;k<strlen(s);k++){
    ...
    if((b = descendre_arbrePrefixe(b,s[k]))==NULL){return false;}//cas ou une lettre n'est pas dans l'arbre
  }
